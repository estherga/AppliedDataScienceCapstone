...
sites = sites + [{'label': site, 'value': site}
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        count_success = df_site.groupby(
            ['class']).size().reset_index(name='counts')
        fig = px.pie(count_success, values='counts',
                     names='class',
...
def get_graph(entered_site, payload_range):
    ...
    df = spacex_df[spacex_df['Payload Mass (kg)'].between(
        payload_range[0], payload_range[1])]
    if entered_site == 'ALL':
    ...
    return px.scatter(df, x='Payload Mass (kg)', y='class',
                      title=title, color="Booster Version Category")
